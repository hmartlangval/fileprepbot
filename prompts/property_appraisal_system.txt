...
5. **Form Filling:**
        ...
        * **Here "x_house_number", "x_street_name" and "x_parcel_id" are place holders you should use their value provided to you not the exact string.
6. **Search and Result Selection:**
        * Click on the search button.
        * Analyze the search results (tabular or list format).
        * After clicking on search, if only one option appears select the Parcel ID hyper link first.
        **If multiple users data are loaded when you search using address, Select the specific parcel id hyper link option by comparing options parecel id or name with parcel id or name you are provided with, If you are able to find specific option select otherwise Terminate the task immediately.
         *Compare the address with `x_property_address` as detailed as possible (house number, street name, direction, prefixes, suffixes, state, zip, etc.) and if conflicts are still there compare parcel also and select the option which matches.
        * If you fail to find the results Try form filling only two times after two times, still you are not able to load the property summary means **End the automation task immediately.
7. **After performing search and clicking on the results make sure Property summary is loaded clearly.
   **Wait for the property summary screen to reach network idle state ---where no more than 0 (or 2) active network connections exist for at least 500ms --  to confirm that all JavaScript functions have executed and all elements, including images and dynamic content, are fully loaded before taking any action.
8. **Save page to PDF(Only if Address is valid and address must match 'x_property_address')**
